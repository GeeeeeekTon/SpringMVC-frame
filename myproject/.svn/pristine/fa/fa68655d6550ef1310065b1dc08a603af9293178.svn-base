...
(function ($) {
    $.jBox = function (content, options) {
        ...
        $.each(content, function (stateName, stateOptions) {
            ...
            if (typeof options.height == 'number') {
				...
				if(contentHeight>$(window).height()-100) contentHeight=$(window).height()-100
            ...
            if (!data.isObject && isRequest) {
				...
				if(loadingHeight>$(window).height()-100) loadingHeight=$(window).height()-100
                if (typeof options.height == 'number') {
            ...
            states.push('<div class="jbox-button-panel" style="height:25px;padding:5px 0 5px 0;text-align: center;' + (withButtons ? '' : 'display:none;') + '">');
            if (!options.isTip) {
        ...
        var setFade = function () {
            if (options.showFade) {
                ...
                $boxFade.css({
                    position: "absolute",
                    height: options.isTip ? $(window).height() : $window.height(),
        ...
        var positionBox = function () {
            if (options.isMessager) {
                ...
                $box.css({
                    position: "absolute",
                    top:($(window).height()-$box.height())/2+"px",// $boxTemp.offset().top + (options.isTip ? $window.scrollTop() : 0),
		...
		//$box.css("marginTop",-1*$box.height()/2+"px")
        return $boxBody;
    ...
    $.jBox.prompt = function (content, title, icon, options) {
        ...
        var html = [];
        html.push('html:');
        html.push('<div style="margin:10px;' + minHeight + 'padding-left:' + (options.icon == 'none' ? 0 : 40) + 'px;text-align:left;padding: 20px 0px 20px 52px;">');
    ...
    $.jBox.tip = function (content, icon, options) {
        ...
        html.push('<span class="jbox-icon jbox-icon-' + options.icon + '" style="position:absolute;top:' + (-10 + options.border) + 'px;left:' + (-9 + options.border) + 'px; width:32px; height:32px;"></span>');
        html.push(options.content);
        html.push('</div>');
        options.content = html.join('');
